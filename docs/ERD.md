...
erDiagram
    ...
    CONCERT_SCHEDULE {
        INT concert_schedule_id PK "콘서트 일정 ID (Primary Key)"
    ...
    CONCERT_SEAT {
        INT concert_seat_id PK "콘서트 좌석 ID (Primary Key)"
        INT concert_schedule_id FK "콘서트 일정 ID (Foreign Key)"
    ...
    CONCERT_RESERVATION {
        INT concert_reservation_id PK "콘서트 예약 ID (Primary Key)"
        INT user_id FK "사용자 ID (Foreign Key)"
        INT concert_schedule_id FK "콘서트 일정 ID (Foreign Key)"
        INT concert_seat_id FK "콘서트 좌석 ID (Foreign Key)"
    ...
    PAYMENT {
        ...
        INT user_id FK "사용자 ID (Foreign Key)"
        INT concert_reservation_id FK "콘서트 예약 ID (Foreign Key)"
